...
import ambulancesimulation.gui.IconsAdjustment;
...
public class Simulation extends Thread {
    ...
    private void simulationSumUp() {
        ...
        this.guiService.removeAmbulanceFromMap();
    ...
    private void findFreeBedsInOtherHospitals(Node startNode, PatientOnMap patient) throws InterruptedException, FileNotFoundException {
    ...
    private synchronized void simulateWayToNextHospital(double distance, PatientOnMap patient, Node targetHospital) throws InterruptedException, FileNotFoundException {
        ...
        addAmbulance(IconsAdjustment.adjustedHospitals.get(targetHospital.getHospital().getHospitalId()-1).getHospitalXCord(), IconsAdjustment.adjustedHospitals.get(targetHospital.getHospital().getHospitalId()-1).getHospitalYCord());
        
        long travelTime = (long) (1 / GUI.simulationSpeed * TIME_SIMULATION_CONST * distance);
        wait(travelTime);
    }

    private synchronized void simulateWayInStraightLine(PatientOnMap patient, Node targetHospital) throws InterruptedException, FileNotFoundException {
        this.guiService.displayActionDescription("Patient with id: " + patient.getPatientId() + " is going to hospital named: " + targetHospital.getHospital().getName());
        addAmbulance(IconsAdjustment.adjustedHospitals.get(targetHospital.getHospital().getHospitalId()-1).getHospitalXCord(), IconsAdjustment.adjustedHospitals.get(targetHospital.getHospital().getHospitalId()-1).getHospitalYCord());        
    ...
    private void addAmbulance(int x, int y) throws FileNotFoundException {
        Platform.runLater( ()-> {
            try {
               this.guiService.displayAmbulance(x, y);
            } catch (FileNotFoundException ex) {
               MessageWindow.displayMessage("Dodawanie karetki na ekran nie powiodło się!");
            }
        }  );
    }
